...
from models.model_satvit_encode import SatViT_Encode
...
def evaluate(data_loader, model, device, criterion, top5=False):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        images = rearrange(images, 'b c (h i) (w j) -> b (h w) (c i j)', i=16, j=16)  # (bs, 196, 3072)
...
def main(args):
    ...
    # 创建SatVit编码器，只进行特征提取
    model = SatViT_Encode(in_dim=io_dim, num_patches=patches_dim, encoder_dim=768, encoder_depth=12, encoder_num_heads=12, head_depth=head_depth)
    # Model Input: [bs, (224/16)**2, (16*16*num_channels)]
    summary(model, input_size=[(8, patches_dim, io_dim)], device="cuda") 
